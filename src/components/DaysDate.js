...
const DaysDate = () => {
  ...
  return (
    <>
      <TouchableWithoutFeedback onPress={closeKeybord} style={{backgroundColor:'yellow', display: 'flex'} }>
        <>
        <TouchableOpacity onPress={closeKeybord} style={{marginTop: 20,flex:20}}>
        <View style={styles.backgound}>
          <View style={styles.container}>
            {DaysOptions.map((item) =>
              <TouchableOpacity
              style={{...styles.diCon , backgroundColor: item.id===day ? '#ff9966'  : '#ffcc99'}}
              onPress={
                ()=>{function123(item.id);}
              }
              key={item.id}
